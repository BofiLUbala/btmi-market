...
export default function CartScreen() {
  ...
  if (!lines.length) return <View style={styles.empty}><Text style={styles.emptyTitle}>Your cart is empty</Text><Text style={styles.muted}>Choose a product and its exact variant to get started.</Text></View>
  return <ScrollView contentContainerStyle={styles.page}><SectionTitle title={`Cart · ${lines.length}`}/>{lines.map((line) => <Card key={line.variantId}><Text style={styles.name}>{line.name}</Text><Text style={styles.muted}>{line.shopName}</Text><View style={styles.row}><Button variant="outline" title="−" onPress={() => setQuantity(line.variantId, line.quantity - 1)}/><Text style={styles.qty}>{line.quantity}</Text><Button variant="outline" title="+" onPress={() => setQuantity(line.variantId, line.quantity + 1)}/><Text style={styles.price}>{(line.price * line.quantity).toLocaleString()} FC</Text></View><Button variant="outline" title="Remove" onPress={() => remove(line.variantId)}/></Card>)}<Card><View style={styles.rowBetween}><Text style={styles.name}>Estimated subtotal</Text><Text style={styles.total}>{subtotal.toLocaleString()} FC</Text></View><Text style={styles.muted}>Final stock, price and points are checked by BTMI before checkout.</Text></Card>{preview.isError && <ErrorState message="Could not validate this cart. Check stock and try again."/>}{preview.isSuccess && <Card><Text style={styles.valid}>Cart validated with the live BTMI backend.</Text></Card>}<Button title={user ? 'Check live price & stock' : 'Sign in to validate cart'} loading={preview.isPending} onPress={() => user ? preview.mutate() : router.push('/auth/login')}/></ScrollView>
}
