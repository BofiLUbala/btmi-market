...
export default function SellerCashScreen() {
  ...
  // Polls continuously — a cash session's totals move with every cash sale, and unlike
  // orders there is no terminal state that lets the screen ever stop refreshing.
  const summary = useQuery({ queryKey: ['seller', 'cashSummary', activeBusiness?.id], queryFn: () => sellerApi.businessCashSummary(activeBusiness!.id), enabled: Boolean(activeBusiness), refetchInterval: POLL_INTERVAL })
  const sessions = useQuery({ queryKey: ['seller', 'cashSessions', activeBusiness?.id], queryFn: () => sellerApi.businessCashSessions(activeBusiness!.id), enabled: Boolean(activeBusiness), refetchInterval: POLL_INTERVAL })
  ...
  return <ScrollView contentContainerStyle={styles.page}>
    ...
    {tab === 'sessions' && (!sessions.data?.length ? <Card><Text style={styles.muted}>{t('seller.cash.noSessionsYet')}</Text></Card> : sessions.data.map((session) => <Card key={session.id}>
      ...
      <Text style={styles.date}>{t('seller.cash.opened')}: {formatDateTime(session.opened_at)}</Text>
