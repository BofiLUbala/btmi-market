...
export default function SellerHome() {
  ...
  if((shops.data?.length ?? 0) === 0) return <View style={styles.center}><Text style={styles.title}>{t('seller.finishSetupTitle')}</Text><Text style={styles.muted}>{t('seller.finishSetupBody', { name: active.name })}</Text><Button title={t('seller.finishSetup')} onPress={()=>router.push('/seller/onboarding')}/><Button variant="outline" title={t('common.backToMarketplace')} onPress={()=>router.replace('/(buyer)')}/></View>
