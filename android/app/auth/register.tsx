...
export default function RegisterScreen() {
  ...
  return <ScrollView contentContainerStyle={styles.page}>
    ...
    <Pressable style={[styles.choice, styles.buyer]} accessibilityRole="button" accessibilityLabel={t('auth.register.createBuyer')} onPress={() => router.push('/auth/register-buyer')}>
      <View style={[styles.icon, { backgroundColor: colors.green }]}><Ionicons name="bag-handle-outline" size={28} color={colors.onGreen} /></View>
    ...
    <Pressable style={[styles.choice, styles.seller]} accessibilityRole="button" accessibilityLabel={t('auth.register.createSeller')} onPress={() => router.push('/auth/register-seller')}>
      <View style={[styles.icon, { backgroundColor: colors.gold }]}><Ionicons name="storefront-outline" size={28} color={colors.ink} /></View>
      <View style={styles.copy}>
...
const makeStyles = (colors: Colors) => StyleSheet.create({
  ...
  choice: { minHeight: 140, borderRadius: radius.md, borderWidth: 2, padding: spacing.md, flexDirection: 'row', alignItems: 'center', gap: spacing.md },
