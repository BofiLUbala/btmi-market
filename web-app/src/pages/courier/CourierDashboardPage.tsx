...
export default function CourierDashboardPage(){
 ...
 function Scanner(){const pickup=current&&['COURIER_ACCEPTED','READY_FOR_PICKUP'].includes(current.delivery_status)&&['READY','READY_FOR_PICKUP'].includes(current.status),delivery=current?.delivery_status==='COURIER_ARRIVED';return <section className="courier-scanner-grid"><ScanCard title="Chez le vendeur" body="Scannez le QR de collecte lorsque la commande est prête." enabled={!!pickup} onClick={()=>current&&scan('PICKUP',current)}/><ScanCard title="Chez l’acheteur" body="Scannez le QR imprimé sur le colis après avoir confirmé votre arrivée." enabled={!!delivery} onClick={()=>current&&scan('DELIVERY',current)}/>{!pickup&&!delivery&&<div className="courier-glass courier-empty-wide">Aucune mission ne nécessite un scan actuellement.</div>}</section>}
