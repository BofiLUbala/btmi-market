  ...
  return (
    <div>
      <div style={{ marginBottom: 20 }}>
        <h2 style={{ fontSize: 20, fontWeight: 800, margin: '0 0 4px' }}>Stock Movement History</h2>
        <p style={{ color: '#94a3b8', fontSize: 13, margin: 0 }}>Audit trail of all inventory changes across all shops.</p>
      </div>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <input placeholder="Variant ID" value={variantId} onChange={(e) => { setVariantId(e.target.value); setPage(0) }}
          style={{ width: 150, padding: '8px 12px', borderRadius: 8, border: '1px solid #334155', backgroundColor: '#0f172a', color: '#f8fafc', fontSize: 13 }} />
        ...
        <input type="date" value={toDate} onChange={(e) => { setToDate(e.target.value); setPage(0) }}
          style={{ padding: '8px 12px', borderRadius: 8, border: '1px solid #334155', backgroundColor: '#0f172a', color: '#f8fafc', fontSize: 13 }} />
        <span style={{ color: '#64748b', fontSize: 12 }}>{total} movements</span>
      </div>

      {loading ? (
        <div style={{ padding: 40, textAlign: 'center', color: '#64748b' }}>Loading...</div>
      ) : movements.length === 0 ? (
        <div style={{ padding: 40, textAlign: 'center', color: '#64748b' }}>No movements found</div>
      ) : (
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ borderBottom: '1px solid #1e293b' }}>
                {['Timestamp', 'Type', 'Shop', 'Variant', 'SKU', 'Before', 'After', 'Delta', 'Reason'].map(h => (
