...
export default function SearchAdminPage() {
  ...
  const tabLabels: Record<'analytics' | 'queries', string> = {
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 4, marginBottom: 16 }}>
        {(['analytics', 'queries'] as const).map(tabKey => (
        ...
        <>
          ...
          {analytics.message && (
            <div style={{ backgroundColor: '#0f172a', border: '1px solid #1e293b', borderRadius: 10, padding: 16 }}>
              <h4 style={{ fontSize: 14, fontWeight: 700, margin: '0 0 8px', color: '#94a3b8' }}>{t('admin.searchAdmin.engineStatus')}</h4>
      ...
      {tab === 'queries' && (
        <div>
          ...
          {queries.length === 0 ? (
            <div style={{ padding: 40, textAlign: 'center', color: '#64748b' }}>{t('admin.searchAdmin.noQueriesLogged')}</div>
          ) : (
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                <thead>
                  <tr style={{ borderBottom: '1px solid #1e293b' }}>
                    {[t('admin.searchAdmin.colTimestamp'), t('admin.searchAdmin.colQuery'), t('admin.searchAdmin.colResultsCount'), t('admin.searchAdmin.colSearchType')].map(h => (
                    ...
                    <tr key={i} style={{ borderBottom: '1px solid #1e293b' }}>
                      ...
                      <td style={{ padding: '10px 12px' }}>
                        <span style={{ fontSize: 11, fontWeight: 700, padding: '2px 8px', borderRadius: 4, backgroundColor: '#1e293b', color: '#93c5fd' }}>
                          {q.search_type || t('admin.searchAdmin.searchTypeText')}
                        </span>
                      </td>
                    </tr>
                  ))}
