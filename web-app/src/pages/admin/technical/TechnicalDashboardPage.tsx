  ...
  return (
    <div style={{ overflowX: 'auto', borderRadius: 8, border: '1px solid #1e293b' }}>
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
        ...
        <tbody>
          {rows.length === 0 ? (
            <tr><td colSpan={columns.length} style={{ padding: 16, textAlign: 'center', color: '#475569' }}>{t('admin.technical.noData')}</td></tr>
          ) : rows.map((row, i) => (
...
export default function TechnicalDashboardPage() {
  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      {activeTab === 'database' && db && (
        <SectionCard title={t('admin.technical.postgresHealthTitle')} icon="🗄️" onRefresh={() => adminTechnicalApi.getPostgresHealth().then(setDb)}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 12, marginBottom: 16 }}>
            ...
            <KPICard label={t('admin.technical.kpiDbSize')} value={db.database_size_formatted || '—'} icon="💾" />
            <KPICard label={t('admin.technical.kpiMigration')} value={db.migration_version} icon="🔄" />
          </div>

          {db.long_running_queries && db.long_running_queries.length > 0 ? (
            <>
              <h4 style={{ fontSize: 13, color: '#fb923c', margin: '0 0 10px' }}>{t('admin.technical.longRunningQueries')}</h4>
              <DataTable
                columns={[t('admin.technical.colPid'), t('admin.technical.colDuration'), t('admin.technical.colState'), t('admin.technical.colQuerySanitized')]}
            ...
            <div style={{ color: '#4ade80', fontSize: 13 }}>{t('admin.technical.noLongRunningQueries')}</div>
          )}
        </SectionCard>
      )}
      ...
      {activeTab === 'redis' && redis && (
        <SectionCard title={t('admin.technical.redisHealthTitle')} icon="⚡" onRefresh={() => adminTechnicalApi.getRedisHealth().then(setRedis)}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 12 }}>
            ...
            <KPICard label={t('admin.technical.kpiConnectedClients')} value={redis.connected_clients} icon="👥" />
            <KPICard label={t('admin.technical.kpiEvictions')} value={redis.eviction_count} icon="♻️" />
            <KPICard label={t('admin.technical.kpiCacheHitRate')} value={`${redis.cache_hit_rate.toFixed(1)}%`} icon="🎯" />
          </div>
        </SectionCard>
      )}
      ...
      {activeTab === 'security' && (
        <SectionCard title={t('admin.technical.tabSecurity')} icon="🔐" onRefresh={() => adminTechnicalApi.getSecurityEvents({ limit: 20 }).then((r) => setSecurityEvents(r.events ?? []))}>
          ...
          {securityEvents.length === 0 && !loading && (
            <div style={{ color: '#4ade80', fontSize: 13, marginTop: 8 }}>{t('admin.technical.noSecurityEvents')}</div>
          )}
        </SectionCard>
      )}
      ...
      {/* ── SESSIONS TAB ─────────────────────────────────────────────────── */}
      {activeTab === 'sessions' && (
        <SectionCard title={t('admin.technical.tabSessions')} icon="🔑" onRefresh={() => adminTechnicalApi.getAdminSessions().then((r) => setSessions(r.sessions ?? []))}>
      ...
      {/* ── VERSIONS TAB ─────────────────────────────────────────────────── */}
      {activeTab === 'versions' && (
        <SectionCard title={t('admin.technical.versionsTitle')} icon="🏷️" onRefresh={() => adminTechnicalApi.getAppVersions().then((r) => setVersions(r.versions ?? []))}>
          ...
          {versions.length === 0 && (
            <div style={{ color: '#64748b', fontSize: 13 }}>{t('admin.technical.noVersionConfigs')}</div>
          )}
        </SectionCard>
      )}
