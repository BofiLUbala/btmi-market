...
export default function AdminActivatePage() {
  ...
  useEffect(() => {
    if (!token) {
      setInvalid('Missing activation token.')
      setVerifying(false)
      return
    }
    adminInvitationApi.verify(token)
      .then((data) => setInvitee(data))
      .catch((err: unknown) => setInvalid(err instanceof Error ? err.message : 'This invitation is invalid or has expired.'))
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: '#090d16', color: '#f8fafc', padding: '24px', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      <div style={{ maxWidth: 440, width: '100%', backgroundColor: '#0f172a', borderRadius: 16, border: '1px solid #1e293b', padding: '36px', boxShadow: '0 20px 50px rgba(0, 0, 0, 0.5)' }}>
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          ...
          <h1 style={{ fontSize: 22, fontWeight: 800, letterSpacing: '0.02em', margin: '0 0 6px' }}>Activate Admin Account</h1>
          <p style={{ color: '#94a3b8', fontSize: 13, margin: 0 }}>Set your password to join the TBK Control Center</p>
        </div>

        {verifying ? (
          <div style={{ textAlign: 'center', padding: '24px 0', color: '#94a3b8', fontSize: 13 }}>Verifying invitation...</div>
        ) : invalid ? (
          <div>
            ...
            <p style={{ fontSize: 12, color: '#64748b', textAlign: 'center' }}>
              Ask a SUPER_ADMIN to resend your invitation, or{' '}
              <Link to="/admin/login" style={{ color: '#60a5fa' }}>return to sign in</Link>.
            </p>
          </div>
        ) : activated ? (
          ...
          <>
            {invitee && (
              <div style={{ backgroundColor: '#1e1b4b', border: '1px solid #3730a3', borderRadius: 8, padding: '10px 14px', marginBottom: 24, fontSize: 12, color: '#c7d2fe' }}>
                ...
                <div>Role: {invitee.role.replace('_', ' ')}</div>
              </div>
            )}
            ...
            <form onSubmit={handleSubmit}>
              <div style={{ marginBottom: 18 }}>
                <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#cbd5e1', marginBottom: 6 }}>New Password</label>
                <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} required minLength={8} placeholder="Minimum 8 characters" style={inputStyle} />
              </div>
              <div style={{ marginBottom: 24 }}>
                <label style={{ display: 'block', fontSize: 13, fontWeight: 600, color: '#cbd5e1', marginBottom: 6 }}>Confirm Password</label>
                <input type="password" value={passwordConfirm} onChange={(e) => setPasswordConfirm(e.target.value)} required minLength={8} placeholder="Repeat password" style={inputStyle} />
              </div>
              <button
                type="submit"
                disabled={submitting}
